from .providers import CoHereProvider, OpenRouterProvider, OpenAIProvider, BaseOpenAIProvider
...
class LLMProviderFactory:
    ...
    def create(self, provider: str):
        ...
        # Create provider (ONLY required args)
        if provider == LLMEnums.OPENAI.value:
            ...
            return CoHereProvider(api_key = self.config.COHERE_API_KEY, 
                                default_input_max_characters = self.config.INPUT_DEFAULT_MAX_CHARACTERS, default_generation_max_output_tokens = self.config.GENERATION_DEFAULT_MAX_TOKENS, default_generation_temperature = self.config.GENERATION_DEFAULT_TEMPERATURE )
    ...
    def _initialize_base(self, instance):
        ...
        if hasattr(instance, "set_generation_model"):
            instance.set_generation_model(self.config.GENERATION_MODEL)
